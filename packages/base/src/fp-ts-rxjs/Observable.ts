...
export const map: <A, B>(
  f: (a: A) => B
) => (fa: Observable<A>) => Observable<B> = (f) => (fa) => fa.pipe(rxMap(f))
...
export const apFirst: <B>(
  fb: Observable<B>
) => <A>(fa: Observable<A>) => Observable<A> = (fb) =>
  flow(
    map((a) => () => a),
    ap(fb)
  )
...
export const apSecond = <B>(
  fb: Observable<B>
): (<A>(fa: Observable<A>) => Observable<B>) =>
  flow(
    map(() => (b: B) => b),
    ap(fb)
  )
...
export const chain: <A, B>(
  f: (a: A) => Observable<B>
) => (ma: Observable<A>) => Observable<B> = (f) => (ma) => ma.pipe(mergeMap(f))
...
export const chainFirst: <A, B>(
  f: (a: A) => Observable<B>
) => (ma: Observable<A>) => Observable<A> = (f) =>
  chain((a) =>
    pipe(
      f(a),
      map(() => a)
...
export const alt: <A>(
  that: () => Observable<A>
) => (fa: Observable<A>) => Observable<A> = (that) => (fa) => merge(fa, that())
...
export const separate: <A, B>(
  fa: Observable<E.Either<A, B>>
) => Separated<Observable<A>, Observable<B>> =
  /*#__PURE__*/
...
export const partition: {
  ...
  <A>(predicate: Predicate<A>): (
    fa: Observable<A>
  ) => Separated<Observable<A>, Observable<A>>
} =
  <A>(p: Predicate<A>) =>
  (fa: Observable<A>) =>
 ...
 * @deprecated
 */
export const observable: Monad1<URI> &
  Alternative1<URI> &
  Filterable1<URI> &
  MonadObservable1<URI> = {
  URI,
  map: map_,
  of,
  ap: ap_,
...
export const bindTo = <K extends string, A>(
  name: K
): ((fa: Observable<A>) => Observable<{ [P in K]: A }>) =>
...
export const bind = <K extends string, A, B>(
  ...
  fa: Observable<A>
) => Observable<{ [P in keyof A | K]: P extends keyof A ? A[P] : B }>) =>
  chain((a) =>
    pipe(
      f(a),
